...
interface EditModalProps {
  ...
  onClose: () => void;
  onSave: (data: Partial<CrudRecord>) => Promise<void>;
}

export function EditModal({ isOpen, record, columns, config, onClose, onSave }: EditModalProps) {
  const [formData, setFormData] = useState<Partial<CrudRecord>>(record || {});
  ...
  return (
    <Modal
      isOpen={isOpen}
      title={`Edit ${config.title || 'Record'}`}
      onClose={onClose}
      size="lg"
      footer={
        <>
          ...
          <div key={column.key}>
            ...
            {column.type === 'boolean' ? (
              ...
              <textarea
                ...
                onChange={e => handleChange(column.key, e.target.value)}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                rows={column.type === 'string' && String(formData[column.key] || '').split('\n').length > 3 ? 4 : 2}
                disabled={loading}
              />
            )}
          </div>
