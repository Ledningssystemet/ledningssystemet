  ...
  if (isAny(businessObject, [ 'bpmn:Lane', 'bpmn:Participant' ]) && isExpanded(element)) {
    ...
    assign(actions, {
      'lane-insert-above': {
        ...
        title: translateString('Add Lane above'),
        action: {
      ...
      if (element.height >= 120) {
        assign(actions, {
          'lane-divide-two': {
            ...
            title: translateString('Divide into two Lanes'),
            action: {
      ...
      if (element.height >= 180) {
        assign(actions, {
          'lane-divide-three': {
            ...
            title: translateString('Divide into three Lanes'),
            action: {
    ...
    assign(actions, {
      'lane-insert-below': {
        ...
        title: translateString('Add Lane below'),
        action: {
          click: function(event, element) {
    ...
    if (is(businessObject, 'bpmn:EventBasedGateway')) {
      ...
      assign(actions, {
        'append.receive-task': appendAction(
          ...
          translateString('Append ReceiveTask')
        ),
        'append.message-intermediate-event': appendAction(
          'bpmn:IntermediateCatchEvent',
          'bpmn-icon-intermediate-event-catch-message',
          translateString('Append MessageIntermediateCatchEvent'),
          { eventDefinitionType: 'bpmn:MessageEventDefinition' }
        ),
        'append.timer-intermediate-event': appendAction(
          'bpmn:IntermediateCatchEvent',
          'bpmn-icon-intermediate-event-catch-timer',
          translateString('Append TimerIntermediateCatchEvent'),
          { eventDefinitionType: 'bpmn:TimerEventDefinition' }
        ),
        'append.condition-intermediate-event': appendAction(
          'bpmn:IntermediateCatchEvent',
          'bpmn-icon-intermediate-event-catch-condition',
          translateString('Append ConditionIntermediateCatchEvent'),
          { eventDefinitionType: 'bpmn:ConditionalEventDefinition' }
        ),
        'append.signal-intermediate-event': appendAction(
          'bpmn:IntermediateCatchEvent',
          'bpmn-icon-intermediate-event-catch-signal',
          translateString('Append SignalIntermediateCatchEvent'),
      ...
      assign(actions, {
        'append.compensation-activity':
            appendAction(
              'bpmn:Task',
              'bpmn-icon-task',
              translateString('Append compensation activity'),
              {
    ...
    if (!is(businessObject, 'bpmn:EndEvent') &&
      ...
      assign(actions, {
        'append.end-event': appendAction(
          ...
          translateString('Append EndEvent')
        ),
        'append.gateway': appendAction(
          'bpmn:ExclusiveGateway',
          'bpmn-icon-gateway-none',
          translateString('Append Gateway')
        ),
        'append.append-task': appendAction(
          'bpmn:Task',
          'bpmn-icon-task',
          translateString('Append Task')
        ),
        'append.intermediate-event': appendAction(
          'bpmn:IntermediateThrowEvent',
          'bpmn-icon-intermediate-event-none',
          translateString('Append Intermediate/Boundary Event')
        )
      });
    }
  }
  ...
  if (!popupMenu.isEmpty(element, 'bpmn-replace')) {
    ...
    assign(actions, {
      'replace': {
        ...
        title: translateString('Change type'),
        action: {
          click: function(event, element) {
            ...
            popupMenu.open(element, 'bpmn-replace', position, {
              title: translateString('Change element'),
              width: 300,
              search: true
            });
          }
  ...
  if (is(businessObject, 'bpmn:SequenceFlow')) {
    assign(actions, {
      'append.text-annotation': appendAction(
        ...
        translateString('Append TextAnnotation')
      )
    });
  }
  ...
  if (is(businessObject, 'bpmn:TextAnnotation')) {
    assign(actions, {
      'connect': {
        group: 'connect',
        className: 'bpmn-icon-connection-multi',
        title: translateString('Connect using Association'),
        action: {
          click: startConnect,
          dragstart: startConnect,
        },
  ...
  if (isAny(businessObject, [ 'bpmn:DataObjectReference', 'bpmn:DataStoreReference' ])) {
    assign(actions, {
      'connect': {
        group: 'connect',
        className: 'bpmn-icon-connection-multi',
        title: translateString('Connect using DataInputAssociation'),
        action: {
          click: startConnect,
          dragstart: startConnect
        }
