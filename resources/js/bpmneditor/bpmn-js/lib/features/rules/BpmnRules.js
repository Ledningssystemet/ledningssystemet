...
function canResize(shape, newBounds) {
  if (is(shape, 'bpmn:SubProcess')) {
    return !newBounds || (newBounds.width >= 75 && newBounds.height >= 75);
  }
  ...
  if (is(shape, 'bpmn:Task')) {
    return !newBounds || (newBounds.width >= 30 && newBounds.height >= 20);
  }
...
function canConnectAssociation(source, target) {
  ...
  // Allow connection between bpmn:DataObjectReference and bpmn:DataStoreReference
  if((is(source, 'bpmn:DataObjectReference') && is(target, 'bpmn:DataStoreReference')) ||
     (is(target, 'bpmn:DataObjectReference') && is(source, 'bpmn:DataStoreReference'))){
     return true;
  }
...
function canConnectDataAssociation(source, target) {
  ...
  if (isAny(source, [ 'bpmn:DataObjectReference' ]) &&
