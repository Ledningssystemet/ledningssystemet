...
export default function BpmnRenderer(
  ...
  function renderAssociation(parentGfx, element, attrs = {}) {
    ...
    try {
       var srctype = semantic.get('sourceRef').$type;
       var targettype = semantic.get('targetRef').$type;

       if ((undefined !== srctype) && (undefined !== targettype))
          svgClasses(parentGfx).add('association-' + srctype.replace(/^bpmn:/, '').toLowerCase() + '-' + targettype.replace(/^bpmn:/, '').toLowerCase());
    }
    catch(e) {       // ignore
    }
...
BpmnRenderer.prototype.drawShape = function(parentGfx, element, attrs = {}) {
  ...
  if('label' == type)
  {
     let parentType = element.businessObject.$type.replace('bpmn:', '').toLowerCase();
     svgClasses(parentGfx)
      .add('data-type-label')
      .add('parent-data-type-'+parentType);
      
     svgAttr(parentGfx, {
        'data-type': type.replace('bpmn:','').toLowerCase(),
        'data-name': name,
  ...
  {
     svgClasses(parentGfx)
      .add('data-type-'+type.replace('bpmn:','').toLowerCase());
