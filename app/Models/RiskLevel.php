...
class RiskLevel extends Model
{
   ...
   public static function getPrettyName($plural = false){ if($plural) { return __("Risk levels"); } else { return __("Risk level"); }}
  ...
  /**
   ...
   {
      return [
         'name' => ['required', Rule::unique('risk_levels')->ignore($this->id)],
         'ordinal' => 'required|numeric',
      ];
   }   

   /**
    * Get risk level by probability and consequence combination
    */
   public static function getRisklevel($probability, $consequence)
   {
      if((null == $probability) ||
         (null == $consequence))
         return null;

      return Cache::rememberForever('risklevel_'.$probability->id.'_'.$consequence->id, function() use ($probability, $consequence) {
         $mappings = DB::table('risk_level_mappings')
            ->where('probability_level_id', $probability->id)
         ...
         return RiskLevel::findOrFail($mappings[0]->risk_level_id);
      });
   }
}
