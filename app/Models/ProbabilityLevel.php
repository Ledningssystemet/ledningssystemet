...
class ProbabilityLevel extends Model
{
   ...
   public static function getPrettyName($plural = false){ if($plural) { return __("Probability levels"); } else { return __("Probability level"); }}
   ...
   public static function boot()
   {
      ...
      static::deleting(function ($model) {
         if($model->int_risks()->count())
            abort(400, __("You may not delete a probability level that is in use"));
      });
   }
   ...
   {
      return [
         'name' => ['required', Rule::unique('probability_levels')->ignore($this->id)],
         'ordinal' => 'required|numeric',
      ];
   }
   
   /**
    * Get the risks with this probability level
    */
   public function int_risks() : HasMany
   {
       return $this->hasMany(Risk::class, 'probability_id');
   }   
}
