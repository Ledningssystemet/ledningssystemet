...
class AvailabilityClass extends Model
{
      
   public static function getPrettyName($plural = false){ if($plural) { return __("Availability classes"); } else { return __("Availability class"); }}
   ...
   {
      return [
         'name' => ['required', Rule::unique('availability_classes')->ignore($this->id)],
   ...
   */
   public function reorder()
   {
      ...
      $afterobj = (null == $after) ? null : AvailabilityClass::findOrFail($after);
      ...
      $ordinal = 2;
      foreach(AvailabilityClass::orderBy('ordinal')->get() as $obj)
      {
         $obj->ordinal = $ordinal;
         $obj->save();
