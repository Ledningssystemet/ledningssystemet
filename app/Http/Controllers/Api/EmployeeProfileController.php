...
{
    /**
     * Ensure the authenticated user may access employee profile data.
     *
     * @throws AccessDeniedHttpException
     */
    private function authorise(Request $request): void
    {
        if (config('ledningssystemet.disable_staff', false)) {
        ...
        if (! $authUser->haveAnyAccessRights(['managementtools.edit'])) {
            throw new AccessDeniedHttpException();
        }
    }

    /**
     * Resolve the target User or abort 404.
     *
    ...
    /**
     * General profile information for the given employee.
     */
    public function show(Request $request, int $userId): JsonResponse
    {
        $this->authorise($request);
        $user = $this->resolveEmployee($userId);
    ...
    /**
     * Roles assigned to the given employee, including their process activities.
     */
    public function roles(Request $request, int $userId): JsonResponse
    {
        $this->authorise($request);
        $user = $this->resolveEmployee($userId);
    ...
    /**
     * Qualifications for the given employee (achieved and missing mandatory ones).
     */
    ...
    /**
     * Competences for the given employee, including role requirements.
     */
    public function competences(Request $request, int $userId): JsonResponse
    {
        $this->authorise($request);
        $user = $this->resolveEmployee($userId);
    ...
    /**
     * Objects the given employee is responsible for.
     */
    public function responsibilities(Request $request, int $userId): JsonResponse
    {
        $this->authorise($request);
        $user = $this->resolveEmployee($userId);
        ...
        return response()->json([
            ...
            'controls'          => Control::whereNull('not_applicable_at')
